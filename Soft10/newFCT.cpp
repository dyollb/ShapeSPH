...
static void reverse_it(float *input,
		       float *output,
		       int n)
{
  int i, len;
...
static void PartitionAdd(float *data,
			 float *result,
			 int n,
			 int k)
{
...
void kFCT(float *data,
		  ...
		  int permflag)
{
	float *lowpoly, *highpoly, *lowptr, *highptr;
	const double *modptr;
	float *temp, dn2;
	int p2, p3;
	unsigned int i, j, levelsize, highsize, lowsize, loopcntr;
	...
	if (permflag == 1)
		...
		memcpy(lowpoly, data, sizeof(float) * n);
	}
	...
	if (k < n) 
		...
		memcpy(result, temp, sizeof(float) * n);
...
static void ChebyDivRem(float *dividend,
			float m0,
			float *remres,
			int n)
{
